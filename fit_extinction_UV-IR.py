...
if 1:
    ...
    if 1:
        ...
        mask = y > 0
        Q.sp_sdss = spectrum(x=x[mask], y=y[mask], err=err[mask], name=qname)

        # correction for galactic extinction
        if Q.AvMW > 0:
            Q.sp_sdss.y *= np.exp(+Q.AvMW * extinction_gordon23(l=np.array(Q.sp_sdss.x), Rv=3.1) / 1.086)
            Q.sp_sdss.err *= np.exp(+Q.AvMW * extinction_gordon23(l=np.array(Q.sp_sdss.x), Rv=3.1) / 1.086)
...
if 1:
    ...
    Q.sp_sdss.mask_fit = Q.sp_sdss.x > 0

    # exclude regions around emission lines
    lines = {}
    lines['Lya'] = em_line('Lya', 1215.67, 40)
    lines['SiIV'] = em_line('SiIV', 1398.3, 25)
    lines['CIV'] = em_line('CIV', 1546.15, 40)
    lines['CIII]'] = em_line('CIII]', 1905.9, 40)
    lines['MgII'] = em_line('MgII', 2800, 50)
    lines['[OII]'] = em_line('[OII]', 3729, 15)
    lines['[NeIII]'] = em_line('[NeIII]', 3869, 15)
    lines['Hg'] = em_line('Hg', 4346, 60)
    ...
    for l in lines.values():
        Q.sp_sdss.mask_fit[np.abs(Q.sp_sdss.x * 1e4 / (1 + Q.z_qso) - l.l) < l.w] = False
    ...
    data_x = np.array(Q.sp_sdss.x[Q.sp_sdss.mask_fit])
    data_y = np.array(Q.sp_sdss.y[Q.sp_sdss.mask_fit])
    data_err = np.array(Q.sp_sdss.err[Q.sp_sdss.mask_fit])

    # normalize to flux at wavelength = w_norm
    data_norm = np.nanmean(Q.sp_sdss.y[np.abs(Q.sp_sdss.x - w_norm) < w_norm_disp])
    data_y /= data_norm
    data_err /= data_norm

    # shift data_x to the absorption restframe
    data_x /= (1 + Q.z_abs)
    print('Nsdss points:',np.sum(Q.sp_sdss.mask_fit))

    # check data for absorption/emission details
    if 1:
        plt.subplots()
        # plot unmasked sdss spectra
        plt.plot(Q.sp_sdss.x, Q.sp_sdss.y / data_norm, color='red', zorder=-10)
        plt.errorbar(x=Q.sp_sdss.x, y=Q.sp_sdss.y / data_norm, yerr=Q.sp_sdss.err / data_norm, ecolor='red', fmt='none', zorder=-10)
        ...
        if 1:
            ...
            plt.plot(s_composite.x * (1 + Q.z_qso), s_composite.y / y_cont_norm, color='black')
...
if 1:
    ...
    def fit_qso_reduced(pars=parameters_reduced, debug=False, photometry=False, return_data = False,show_archival=True):
        ...
        # define weight function for spectral data highlighted 2175 bump region
        ...
        if debug:
            print(pars)
            fit_composite = interp1d(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), f0*s_composite.y,
                                        fill_value='extrapolate')
            ...
            ax[1].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), f0*s_composite.y , label='Cont')
            ax[1].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), fit_model, label='Cont+Ext', color='red')
            ax[1].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), f0 * np.array(s_composite.y) *
            ...
            ax[0].axvline(w_norm / (1 + Q.z_abs), ls='--')
            ax[1].axvline(w_norm / (1 + Q.z_abs), ls='--')
            if show_archival:
                ext_model_archival = extinction_quasar_reduced(l=s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                                                      c1=Q.pars_arch[1], c2=Q.pars_arch[2], c3=Q.pars_arch[3], x0=Q.pars_arch[4], gamma=Q.pars_arch[5])
                ext_model_archival_no_bump = extinction_quasar_reduced(l=s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                                                               c1=Q.pars_arch[1], c2=Q.pars_arch[2], c3=0,
                                                               x0=Q.pars_arch[4], gamma=Q.pars_arch[5])
                #fnorm =  np.mean((s_composite.y * np.exp(-ext_model_archival / 1.086))[mask_norm])
                print('archival fit theta:', Q.pars_arch)
                ax[0].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), ext_model_archival, color='green')
                ax[0].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), ext_model_archival_no_bump, color='green',ls='--')
                ax[1].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), Q.pars_arch[0]*s_composite.y * np.exp(-ext_model_archival / 1.086)/fnorm,
                           color='green')
                ax[1].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                           Q.pars_arch[0] * s_composite.y * np.exp(-ext_model_archival_no_bump / 1.086) / fnorm,
                           color='green',ls= '--')
...
# find the best fit value for the scaling coefficient using the archival fit for extinction
if 1:
    s_composite = calc_composite_cont()
    # normalize composite at flux at the w_norm wavelength
    mask_norm = np.abs(s_composite.x * (1 + Q.z_qso) - w_norm) < w_norm_disp
    s_composite.y /= np.nanmean(s_composite.y[np.abs(s_composite.x * (1 + Q.z_qso) - w_norm) < 0.05])
    # extinction curve for the archival fit
    ext_model_archival = extinction_quasar_reduced(l=s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                                                   c1=Q.pars_arch[1], c2=Q.pars_arch[2], c3=Q.pars_arch[3],
                                                   x0=Q.pars_arch[4], gamma=Q.pars_arch[5])
    fit_model_arch = interp1d(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), s_composite.y * np.exp(-ext_model_archival / 1.086) / fnorm)
    scaling_array = np.linspace(0.0, 10, 500)
    # add the same weight function as for mcmc fit
    weight = np.ones_like(data_x)
    weight[np.abs(data_x - 2175) < 400] = 1
    chiq_x = [np.sum(weight * np.power((data_y - fit_model_arch(data_x) * el) / (data_err), 2)) for el in  scaling_array]
    f_interp = interp1d(chiq_x - np.nanmin(chiq_x), scaling_array)
    Q.pars_arch[0] = f_interp(0)
    chi_arch = np.nanmin(chiq_x)
    print('archival', Q.pars_arch, 'chiq:',np.log10(chi_arch))
    if 1:
        plt.subplots()
        plt.plot(scaling_array,chiq_x )
        plt.subplots()
        plt.plot(Q.sp_sdss.x / (1 + Q.z_abs), Q.sp_sdss.y / data_norm, label='Data', color='black', lw=1, zorder=-100)
        plt.plot(data_x,fit_model_arch(data_x)*Q.pars_arch[0],c='red')
        if 1:
            ext_model_archival_no_bump = extinction_quasar_reduced(l=s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                                                                   c1=Q.pars_arch[1], c2=Q.pars_arch[2], c3=0,
                                                                   x0=Q.pars_arch[4], gamma=Q.pars_arch[5])
            fit_model_arch_no_bump = interp1d(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs),
                                              s_composite.y * np.exp(-ext_model_archival_no_bump / 1.086) / fnorm)
            plt.plot(data_x, fit_model_arch_no_bump(data_x) * Q.pars_arch[0], c='green')
    ...
    if 1:
        ...
        if 1:
            ...
            for i in range(ndim):
                ...
                ax[i].set_ylim(np.nanmin(chi2)-1,np.nanmin(chi2)+10)

            print('theta_best',theta_best)
            print('log chiq:',np.log10(np.nanmin(chi2)))
            plt.show()
        ...
        fit_qso_reduced(pars=theta_for_plot, debug=True)
        plt.show()
